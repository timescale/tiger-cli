...
// FetchServiceSchema opens a read-only connection to the service and
// introspects its schema. It is the shared entry point for the `tiger db
// schema` CLI command and the db_schema MCP tool.
//
...
func FetchServiceSchema(ctx context.Context, service api.Service, role string, pooled bool, opts SchemaOptions) (*DatabaseSchema, error) {
	...
	conn, err := ConnectToService(ctx, service, ConnectionDetailsOptions{
		Pooled:       pooled,
		Role:         role,
		WithPassword: true,
		ReadOnly:     true,
