...
// buildServiceBackupsCmd creates the backups command for listing a service's
...
Examples:
  ...
  tiger service backups

  # List backups for a specific service
  tiger service backups svc-12345

  # Output as JSON
  tiger service backups -o json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resp, err := client.GetBackupsWithResponse(ctx, projectID, serviceID)
			if err != nil {
...
func outputBackupsTable(backups []api.Backup, output io.Writer) error {
	...
	for _, backup := range backups {
		table.Append(
			backup.StartedAt.Format("2006-01-02 15:04"),
			string(backup.Type),
