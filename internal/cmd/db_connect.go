...
func selectConnection(
	...
	opts.ReadOnly = opts.ReadOnly || chosen.ReadOnlySession(cfg)
...
func connectWithPasswordMenu(
	...
	// Rotating the master password is a control-plane write, so read-only mode
	// hides the option rather than offering what it would refuse.
