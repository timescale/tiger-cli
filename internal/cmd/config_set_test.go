...
func TestConfigSetCmd(t *testing.T) {
	tests := []cmdTest{
		...
		},
		{
			...
			check:      checkConfigFile(map[string]any{"api_url": "https://new.api.com/v1"}),
		},
		{
			name:       "set service_id",
			args:       []string{"config", "set", "service_id", "new-service"},
			wantStdout: "Set service_id = new-service\n",
			check:      checkConfigFile(map[string]any{"service_id": "new-service"}),
		},
		{
			name:       "set output",
		...
		},
		{
			...
			check:      checkConfigFile(map[string]any{"output": "yaml"}),
		},
	}
