...
func TestServiceLogsCmd(t *testing.T) {
	...
	runCmdTests(t, []cmdTest{
		...
		},
		{
			...
			args:     []string{"service", "logs", "svc-12345"},
			synctest: true,
			mock:     setupLogs(api.ServiceLogs{}),
		},
		{
			name:     "text output",
			args:     []string{"service", "logs", "svc-12345"},
			synctest: true,
			mock:     setupLogs(api.ServiceLogs{Entries: &entries}),
		...
		},
		{
			...
			args:     []string{"service", "logs", "svc-12345"},
			synctest: true,
			mock:     setupLogs(api.ServiceLogs{Entries: &timestampedEntries}),
...
`,
		...
		},
		{
			...
			synctest: true,
			mock: setupLogs(api.ServiceLogs{Entries: &[]api.ServiceLogEntry{
