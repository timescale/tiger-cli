...
func CheckReadOnlyByServiceID(ctx context.Context, cfg *config.Config, client api.ClientWithResponsesInterface, projectID, serviceID string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("cannot verify whether service %q is tagged PROD (read_only=%s): %w", serviceID, cfg.ReadOnly, err)
	}

	return CheckReadOnly(cfg, ServiceEnvironmentTag(*service))
}
