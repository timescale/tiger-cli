...
func GetConnectionDetails(cfg *config.Config, service api.Service, opts ConnectionDetailsOptions) (*ConnectionDetails, error) {
	return GetConnectionDetailsFor(cfg, service, service, opts)
}
...
func GetConnectionDetailsFor(cfg *config.Config, connService, credService api.Service, opts ConnectionDetailsOptions) (*ConnectionDetails, error) {
	...
	return buildConnectionDetails(cfg, connService.Endpoint, connService.ConnectionPooler, credService, opts)
}
...
func ConnectTarget(ctx context.Context, cfg *config.Config, target *ConnectionTarget, opts ConnectionDetailsOptions, mode pgx.QueryExecMode) (*pgx.Conn, error) {
	details, err := target.Details(cfg, opts)
	if err != nil {
		return nil, err
	}
...
func buildConnectionDetails(cfg *config.Config, direct *api.Endpoint, pooler *api.ConnectionPooler, passwordService api.Service, opts ConnectionDetailsOptions) (*ConnectionDetails, error) {
	endpoint := direct
	isPooler := false
	...
	if opts.WithPassword {
		...
		} else if password, err := GetPassword(cfg, passwordService, opts.Role); err == nil {
