...
- **`internal/`** - All core application logic (non-public Go packages).
  - **`internal/cmd/`** - Cobra command implementations for all CLI commands (auth, project, service, db, config, mcp, feedback, version, upgrade, completion). Each command lives in its own file, named to match the command in snake_case (see [Command Architecture](#command-architecture)). `root.go` holds the root command, global flags, and command tree construction; files ending in `_helper.go` contain cross-group helpers rather than commands.
...
Before committing, run `go tool gofumpt -l -w .`, `go vet ./...`, `go fix -diff ./...`, `go tool staticcheck ./...`, `go tool govulncheck ./...`, and `go test ./...` (gofumpt, staticcheck, and govulncheck are declared as build-time tools in `go.mod`'s `tool` block). Code is formatted with gofumpt, a stricter superset of gofmt, so use it in place of `go fmt` — CI fails on any file it would change.
...
- A case that waits on a timer — anything reaching `common.WaitForService`'s poll loop via `--wait` — sets `synctest: true` to run inside a `testing/synctest` bubble. Time there is virtual, so write the realistic durations (a one-second poll interval, a thirty-second timeout) rather than shrinking them to keep the test fast. The bubble's clock also always starts at the same instant, so a case whose request depends on `time.Now()` (`tiger service logs` sends it as the default upper bound) sets `synctest` too and spells that time out as a plain literal. Don't combine `synctest` with a loopback `httptest.NewServer`: real network I/O never durably blocks, so the bubble hangs until the test binary's own timeout.
...
- A stub works like a mock expectation: it takes the arguments it expects up front and diffs the whole struct against them when called, rather than checking fields one by one or recording what it received for a later check to inspect — `withExecuteQuery` in `db_query_test.go` is the model.

### MCP Tools

MCP tool tests live in `internal/mcp`, one test file per tool file, table-driven on the shared harness in `main_test.go`, which mirrors the command harness in shape, naming, and layout:

- `runTool(t, tool, args, setupMock, opts...)` builds a real server with `NewServer` over the generated mock API client and an isolated `t.TempDir()` config directory (analytics off, docs proxy disabled), connects an in-memory MCP client, and calls the tool through the SDK's actual request path, so schema validation, middleware, and elicitation round trips are all exercised. It returns the `CallToolResult`, the transport error if the call itself broke, and the prompt the tool raised, if any.
- Test cases use the `toolTest` struct and run through `runToolTests` — one inline table per test function, as with `cmdTest`, and with the same field names: `setupMock` registers mock expectations, `opts` configures the run, `checks` holds extra assertions. `wantErr` holds the exact text of the error result and `wantOutput` the exact structured content; left unset, they assert success and no structured content respectively. A case that calls a gated tool without the experimental gate never reaches a handler, so instead of a result it asserts the SDK's transport error with `wantCallErr` (exact text).
- Options configure the run, the counterparts of the command tests' `with*` options: `withConfig` seeds the config file the server starts with, which also decides which tools are registered; `withConfigAfterStart` rewrites it once the server is running, for behavior driven by a config change the per-request reload picks up; `withClientError`/`withNotLoggedIn` make the client factory fail; `withExperimental` turns on the experimental gate so the gated tools are registered; `withClientCapabilities` overrides what the MCP client advertises; and `withKeyringError` makes password storage fail. As in the command tests, a stub of package-level state is a `with*` option built on `withSetup`, written under the stub rule above (`withFetchServiceSchema` in `db_schema_test.go` is the model), never a bare hook in one case.
- A case that expects the tool to prompt sets `wantPrompt` (the `ElicitParams` as the client receives them) and answers it with `withElicitation` (the user's `ElicitResult`). That option is also what gives the client elicitation support, so a case without it proves the tool copes with a client that can't prompt.
- `synctest: true` works as in the command tests, both for a case that reaches `common.WaitForService`'s poll loop via a tool's `wait` parameter and for one whose request depends on `time.Now()`.
- Mock expectations follow the same rules as the command tests (`validCtx`, exact request structs, `httpResponse`, `sampleService`, `sampleReplica`, `expectGetService`); a tool that prompts runs its handler twice per call, so it expects its lookups with an explicit `Times`. As with `runCommand`, every case starts with a fresh in-memory keyring.
