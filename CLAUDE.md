...
**Sensitive data must never reach analytics.** The `ignore` list in `internal/analytics/analytics.go` filters flag and tool-parameter names (write flag names with underscores: `public-key` → `public_key`). When adding a command or tool that handles passwords, keys, SQL queries, connection strings, or similar, add the field names there. All positional arguments are tracked automatically, so accept sensitive values via flags instead. Where a positional argument is unavoidable and may carry sensitive text, annotate the command with `annotationRedactArgs` (`internal/cmd/root.go`) and `wrapCommands` replaces its argument values with `[REDACTED]`, keeping the argument count and the flags — `tiger feedback` is the model.
