...
- Beyond the call itself, a case configures the run through its fields, the counterparts of the command tests' `with*` options: `config` seeds the config file the server starts with, which also decides which tools are registered; `configAfterStart` rewrites it once the server is running, for behavior driven by a config change the per-request reload picks up; `clientErr` makes the client factory fail, so the handler sees a not-logged-in App; and `experimental` turns on the experimental gate so the gated tools are registered.
- `setup` holds t-scoped hooks that run before the server is built. Use one to replace a package-level `var` for a single case, the same seam the command tests reach through a `withSetup`-based option, and write it under the stub rule above: it takes the expected arguments and diffs the whole struct (`stubFetch` in `db_schema_test.go` is the model). `checks` holds extra assertions on state the tool left behind (`checkDefaultService`).
- A case that calls a gated tool without `experimental` never reaches a handler, so instead of a result it asserts the SDK's transport error with `wantCallErr` (exact text).
- `synctest: true` works as in the command tests for a case that reaches `common.WaitForService`'s poll loop, via a tool's `wait` parameter. It also serves a case whose request depends on the current time (`service_logs` sends `time.Now()` as the upper bound): the bubble's clock starts at a fixed instant, so the expected request can be spelled out as a plain literal.
- Mock expectations follow the same rules as the command tests (`validCtx`, exact request structs, `httpResponse`, `sampleService`); a tool that prompts runs its handler twice per call, so it expects its lookups with an explicit `Times`. As with `runCommand`, every case starts with a fresh in-memory keyring.
