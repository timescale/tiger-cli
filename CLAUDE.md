...
Before committing, run `go tool gofumpt -w .`, `go vet ./...`, `go fix -diff ./...`, `go tool staticcheck ./...`, `go tool govulncheck ./...`, and `go test ./...` (gofumpt, staticcheck, and govulncheck are declared as build-time tools in `go.mod`'s `tool` block). Code is formatted with gofumpt, a stricter superset of gofmt, so use it in place of `go fmt` — CI fails on any file it would change.
