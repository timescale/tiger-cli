...
```bash
# Regenerate the API client, mocks, and CLI reference docs
go generate ./...
...
`docs/cli/` is generated by `cmd/generate-docs`, which walks the Cobra command
tree and emits one Markdown page per command. It runs as part of
`go generate ./...`, so regenerate after changing a command, flag, or help text.
Run `go run ./cmd/generate-docs -help` for other output formats and options.

## Development Best Practices

1. **Always format with gofumpt** (`go tool gofumpt -l -w .`) after making file changes and before committing; it's a stricter superset of `go fmt`, and CI fails on unformatted files
...
6. **Run gofumpt** to format your code: `go tool gofumpt -l -w .`
